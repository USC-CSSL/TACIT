...
public class NaiveBayesClassifier {
	...
	public String predict(ArrayList<String> trainingClasses, ArrayList<String> testingClasses, String outputDirectory,
		...
		for (String classPath : trainingClasses) {
			tempOutputPath += classPath.substring(classPath.lastIndexOf(System.getProperty("file.separator")) + 1)
					+ "_";
			tempTrainDirs += classPath + ",";
		}
		String outputPath = outputDir + System.getProperty("file.separator")
				+ tempOutputPath.substring(0, tempOutputPath.length() - 1) + "-" + df.format(dateObj);
		...
		ArrayList<String> tempT2vArgs = new ArrayList<String>(
				Arrays.asList("--input", "--output", tempoutputPath + ".train", "--keep-sequence", keepSeq,
						"--remove-stopwords", stopWords, "--preserve-case", preserveCase));
		...
		ArrayList<String> tempT2vArgsTest = new ArrayList<String>(Arrays.asList("--input", "--output",
				tempoutputPath + ".test", "--keep-sequence", keepSeq, "--remove-stopwords", stopWords,
				"--preserve-case", preserveCase, "--use-pipe-from", tempoutputPath + ".train"));
		// add all the class paths to the argument
		tempT2vArgsTest.addAll(1, Arrays.asList(testDirs));
		String[] t2vArgs_test = Arrays.copyOf(tempT2vArgsTest.toArray(), tempT2vArgsTest.toArray().length,
				String[].class);

		String[] v2cArgs = { "--training-file", tempoutputPath + ".train", "--testing-file", tempoutputPath + ".test",
				"--output-classifier", tempoutputPath + ".out" };
		...
		ConsoleView.printlInConsoleln("\nCreated classifier output file " + tempoutputPath + ".out");
	...
	public void classify(ArrayList<String> trainingClasses, String classificationInputDir,
			String classificationOutputDir, boolean removeStopwords, boolean doLowercase, Date dateObj)
					throws FileNotFoundException, IOException, EvalError {
		DateFormat df = new SimpleDateFormat("MM-dd-yy-HH-mm-ss");
		...
		String outputPath = classificationOutputDir + System.getProperty("file.separator")
				+ tempOutputPath.substring(0, tempOutputPath.length() - 1) + "-" + df.format(dateObj);
		...
		// java 1.6 or greater
		String[] t2vArgs = Arrays.copyOf(tempT2vArgs.toArray(), tempT2vArgs.toArray().length, String[].class);
		String[] t2vArgs_test = { "--input", classificationInputDir, "--output", tempoutputPath + ".test",
				"--keep-sequence", keepSeq, "--remove-stopwords", stopWords, "--preserve-case", preserveCase,
				"--use-pipe-from", tempoutputPath + ".train" };
		String[] v2cArgs = { "--training-file", tempoutputPath + ".train", "--testing-file", tempoutputPath + ".test",
				"--output-classifier", tempoutputPath + ".out", "--report", "test:raw" };
		...
		TacitUtility.createRunReport(classificationOutputDir, "Naive Bayes's", dateObj, null);
	}
