package edu.usc.cssl.tacit.common.ui;

public interface IPreprocessorSettingsConstant {
	
	public static String INITIAL = "initial";
	public static String REMOVE_STOPS = "removeStopWords";
	public static String STOP_PATH = "stop_words_path";
	public static String DELIMETERS = "delimeters";
	public static String STEMMING = "isStemming";
	public static String LANGUAGE = "language";
	public static String INPUT_DICT_LANGUAGE = "input_dict_language";
	public static String LOWER_CASE = "islower_case";
	public static String PRE_PROCESSED = "ispreprocessed";
	public static String OUTPUT_PATH = "pp_output_path";
	public static String LATIN_STEMMER = "latin_stemmer";
	public static String SPELL_CHECK = "spell_check";
	public static String DICTIONARY_PATH="dictionary_path";
	public static String STEM_DICTIONARY = "stem_dictionary";
}
