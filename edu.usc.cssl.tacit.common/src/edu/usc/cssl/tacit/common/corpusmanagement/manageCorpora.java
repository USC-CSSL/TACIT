...
public class manageCorpora {
	...
	public static void saveCorpus(ArrayList<Corpus> corporaList) {
		for (Corpus corpus : corporaList) {
			...
			if (!(new File(corpusLocation).exists())) {
				...
				jsonObj.put("corpus_name","corpusID");
				...
				for (int i=0; i<numClasses; i++) {
					...
					String[] dirParts = currClass.getClassName().split(System.getProperty("file.separator"));
					String dirName = dirParts[dirParts.length-1];
					classObj.put("tacit_loc", corpusLocation+System.getProperty("file.separator")+dirName);
