...
public class HierarchicalLDA {
    ...
    public HierarchicalLDA (String outputDirectory, IProgressMonitor monitor) {
		alpha = 10.0;
		gamma = 1.0;
		eta = 0.1;
	...
	*/
    ...
    public void printNodesToFile() throws IOException {
    	
    	fw = new FileWriter(new File(outputDirectory + File.separator + "GeneratedTopics.txt"));
    	printNodeToFile(rootNode, 0, false);
    	fw.close();
    }
       
    public void printNodeToFile(NCRPNode node, int indent, boolean withWeight) throws IOException {
		...
		out.append("Topic at level "+node.level+" comprises of the following words: ");
		out.append(node.getTopWords(numWordsToDisplay, withWeight)+"\n");
		fw.write(out.toString());
	
		for (NCRPNode child: node.children) {
			printNodeToFile(child, indent + 1, withWeight);
		}
    }
